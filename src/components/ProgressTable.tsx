...
import { deleteProgress } from "@/lib/progress-actions";
...
export const ProgressTable = ({ progress }: { progress: SerializedProgress[] }) => {
  const [query, setQuery] = useState("");
  const baseUrl = (process.env.NEXT_PUBLIC_APP_URL ?? "").replace(/\/$/, "");
  const buildPublicUrl = (slug: string) => (baseUrl ? `${baseUrl}/progress/${slug}` : `/progress/${slug}`);

  const normalized = useMemo(
    () =>
      progress.map((item) => ({
        ...item,
        nameEn: normalizeProgressData(item.dataEn).client.name,
        nameAr: normalizeProgressData(item.dataAr).client.name
      })),
    [progress]
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Work Plan Progress</h1>
          <p className="text-sm text-slate-500">Manage client progress pages.</p>
        </div>
        <Link
                ...
                <tr key={item.id} className="border-b border-slate-100 text-slate-700">
                  ...
                  <td className="py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link
                        href={`/dashboard/progress/${item.id}/edit`}
